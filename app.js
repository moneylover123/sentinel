...
async function runAll() {
  ...
  try { 
    await fetchStockTwits(); 
  } catch(e) { 
    console.error('StockTwits error:', e);
  }
...
async function fetchNews() {
  try {
    ...
    if (!articles.length) { 
      list.innerHTML = '<div class="news-placeholder">No headlines found</div>'; 
      return; 
    }
...
async function fetchStockTwits() {
  try {
    ...
    console.error('StockTwits error:', e);
    document.getElementById('st-bull-pct').textContent = '—';
    document.getElementById('st-bear-pct').textContent = '—';
  }
}
...
async function fetchReddit() {
  try {
    ...
    console.error('Reddit error:', e);
    document.getElementById('rd-bull-pct').textContent = '—';
    document.getElementById('rd-bear-pct').textContent = '—';
  }
}
...
async function fetchMacro() {
  ...
  for (const t of tickers) {
    try {
      ...
      console.error(`Macro ${t.id} error:`, e);
...
function timeAgo(date) {
  const diff = Math.floor((Date.now() - date) / 100
