...
// ============================================================
// STRICT PATTERN DETECTION â€” MAX 10 STRONG PATTERNS ONLY
// ============================================================
function detectPatterns(candles) {
  ...
  for (let i = 4; i < n; i++) {
    ...
    // 1. HAMMER â€” strict: long lower wick 3x body, tiny upper wick, body above midpoint, after downtrend
    if (
    ...
    // 2. SHOOTING STAR â€” strict: long upper wick 3x body, tiny lower wick, after uptrend
    if (
      ...
      scored.push({ time: curr.time, position: 'aboveBar', color: '#ff3366', shape: 'arrowDown', text: 'SHOOTâ˜…', score: 90 });
    }

    // 3. DOJI â€” strict: body less than 5% of range, significant range (not tiny candle)
    if (
    ...
    // 4. BULLISH ENGULFING â€” strict: current body must be 1.5x larger than previous body
    if (
      ...
      scored.push({ time: curr.time, position: 'belowBar', color: '#00e676', shape: 'arrowUp', text: 'â–²ENGULF', score: 95 });
    }

    // 5. BEARISH ENGULFING â€” strict: same rules
    if (
      ...
      scored.push({ time: curr.time, position: 'aboveBar', color: '#ff3366', shape: 'arrowDown', text: 'â–¼ENGULF', score: 95 });
    }

    // 6. MORNING STAR â€” strict 3 candle reversal
    if (
      ...
      scored.push({ time: curr.time, position: 'belowBar', color: '#00e676', shape: 'arrowUp', text: 'MORNINGâ˜…', score: 92 });
    }

    // 7. EVENING STAR â€” strict 3 candle reversal
    if (
      ...
      scored.push({ time: curr.time, position: 'aboveBar', color: '#ff3366', shape: 'arrowDown', text: 'EVENINGâ˜…', score: 92 });
    }

    // 8. DOUBLE TOP â€” only flag if two peaks within 0.2% of each other
    if (i >= 20) {
    ...
    // 9. DOUBLE BOTTOM â€” only flag if two troughs within 0.2% of each other
    if (i >= 20) {
      const seg = c.slice(i - 20, i);
    ...
    // 10. BULL FLAG â€” strong pole then tight consolidation
    if (i >= 8) {
  ...
  // Deduplicate by time â€” keep highest score per candle
  const byTime = {};
  scored.forEach(m => {
...
async function fetchStockTwits() {
  try {
    ...
    document.getElementById('st-bull-pct').textContent = `â–² ${bullPct}%`;
    document.getElementById('st-bear-pct').textContent = `â–¼ ${bearPct}%`;
  } catch(e) {}
}
...
async function fetchReddit() {
  try {
    ...
    document.getElementById('rd-bull-pct').textContent = `â–² ${bullPct}%`;
    document.getElementById('rd-bear-pct').textContent = `â–¼ ${bearPct}%`;
  } catch(e) {}
}
...
async function fetchMacro() {
  ...
  for (const t of tickers) {
    try {
      ...
      document.getElementById(t.id).textContent = 'â€”';
    }
  }
}
...
Respond ONLY in this exact JSON format:
{
  ...
  "evidence": [
    ...
    document.getElementById('evidence-list').innerHTML = analysis.evidence.map(ev => {
      const icon = ev.type === 'bull' ? 'â–²' : ev.type === 'bear' ? 'â–¼' : 'â—';
